...
<body>
    @php
      $payslipChunks = array_chunk($payslip->toArray(), 2); 
    @endphp
    @foreach ($payslipChunks as $chunk)
      <div class="col-quarter">
        <table>
          ...
          @php
              ...
              $emp_id = $pay['emp_id'] ?? null;
              $deductionsForEmp = $deductions[$emp_id] ?? [];
          @endphp
          <th>
            <table class="table table-striped table-bordered landscape-table" style="table-layout: auto; width: 100%; max-width: none;">
                ...
                <tbody>
                  @foreach ($deductionsForEmp as $deduction)
                      @php
                        $eml = $deduction->eml;
                        ...
                        $health = $deduction->health;
                        $mpl = $deduction->mpl;
                        $prem = $deduction->prem;
                        $mp2 = $deduction->mp2;
                        $house_loan = $deduction->house_loan;
                        ...
                        $sumdeduct = $eml + $pol_gfal + $consol + $loan + $ed_asst_mpl + $rlip + $gfal + $computer + $health + $mpl + $prem + $mp2 + $house_loan + $calam_loan + $philhealth + $holding_tax + $lbp + $cauyan + $nsca_mpc + $med_deduction + $grad_guarantor + $cfi + $csb + $projects + $fasfeed + $dis_unliquidated;
                      @endphp
                  @endforeach
                  ...
                  <tr>
                    ...
                    <td width="25%" style="border-bottom: 1px solid #97a8b9;">{{ number_format($rlip, 2)  }}</td>
                    <td width="25%"></td>
                  </tr>
                  <tr>
                    <td width="25%"></td>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td width="25%"></td>
                    <td width="25%"></td>
                    <td width="25%"><span  style="float: right; font-size: 12px;">{{ number_format($sumdeduct, 2) }}</span></td>
                  </tr>
                  <tr>
                    <td width="25%" style="font-weight: 800px;"><strong>NET TAKE HOME PAY</strong></td>
                    <td width="25%"></td>
                    <td width="25%"></td>
                    <td width="25%"><span  style="float: right; color: red; font-size: 12px;">{{ number_format($earnperiod  - $sumdeduct, 2) }}</span></td>
                  </tr>
                  <tr>
